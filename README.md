...
│   │   ├── auth.ts          # 账号注册/登录（PBKDF2 密码哈希 + HMAC 令牌）
...
同步是**本地优先**的——未配置同步服务器时，应用完全离线可用；配置后点击顶栏「同步」按钮，按提示用**用户名 + 密码登录**（或注册）账号，之后多台设备登录同一账号即可共享同一份云端数据（基于 `updatedAt` 的最后写入优先，删除以软删除 `deletedAt` 形式传播）。
...
> 鉴权说明：采用**账号体系**（公网部署必备）。注册时服务端用 salted **PBKDF2-SHA256** 存储密码哈希，登录成功后下发 HMAC 签名令牌（payload 为 `accountId`），服务端无状态校验。同一账号的所有设备共享 `sync:<accountId>:*` 命名空间，因此真正跨设备同步（而非按设备隔离的备份）。
>
> ⚠️ 改了同步后端代码（如本次账号改造）后，需重新 `npm run deploy`（Cloudflare Worker）并重新 `npm run build`（前端）才能让公网生效；仅改代码不会自动更新云端。

## 数据备份与迁移

由于数据存于浏览器 IndexedDB，换设备或清缓存会丢失本地副本；但**登录同一账号后，云端数据会在任意设备拉回**。建议：

- 用账号登录开启云端同步（见上），多设备自动共享同一份数据；
- ⚠️ 本次账号改造把云端命名空间从「按设备」改为「按账号」。若之前用旧版（无账号）同步过，那些 `sync:<deviceId>:*` 旧数据不会自动合并到新账号空间；重新登录账号并首次同步即可（会把当前本地数据作为云端基线上传）；
